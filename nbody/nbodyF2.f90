...
contains
  ...
  do i = 1, nb
     do j = i + 1, nb
        ...
        mag(m) = (dx(1)**2 + dx(2)**2 + dx(3)**2) ** (-1.5d0) * tstep
        m = m+1
     end do
  end do
  m = 1
  do i = 1, nb
     do j = i + 1, nb
        dx = (x(1:3,i) - x(1:3,j))
        v(1:3,i) = v(1:3,i) - mass(j) * mag(m) * dx
        v(1:3,j) = v(1:3,j) + mass(i) * mag(m) * dx
        m = m+1
     end do
!     x(1:3,i) = x(1:3,i) + tstep * v(1:3,i)
  end do
  ...
  pure function energy(x, v, mass)
    ...
    do i = 1, nb
       ...
       do j = i + 1, nb
          ...
          distance = sqrt(dx(1)**2 + dx(2)**2 + dx(3)**2)
